    ...
    def my_forward(
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                start_memory = measure_memory_usage(2)
                ...
                module_time.append(time.time() - start_time)
                module_mem.append(measure_memory_usage(device) - start_memory)
            # print("model idx",idx)
            # print('--'*50)
...
# 加载模型和tokenizer
# module_id = 4
model = LlamaForCausalLM.from_pretrained("../Models/Llama-2-7b-chat-hf").half().to(device2)
...
LlamaModel.forward = create_monitor_forward()
...
inputs = tokenizer(prompts, return_tensors="pt", padding=True, truncation=True).to(device2)  # 将输入转移到GPU
...
# 将module_time 和 module_mem写入到一个文件中


with open("output.txt", "w") as file:
    file.write("======= module time ==========\n")
    file.write(f"{module_time}\n")
    file.write("======= module mem ==========\n")
    file.write(f"{module_mem}\n")
