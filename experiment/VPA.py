...
from utils.utils import create_VPA_offload_forward,create_HPA_copy_forward
import time
import copy
all_time_avg = []
deploy_time_avg = []
...
for j in list(range(10, 121, 10)):
    ...
    for i in range(5):
       ...
       # model = LlamaForCausalLM.from_pretrained(model_id,offload_folder="offload").bfloat16().to(device0)
        ...
        module_id = 4
        model.model.layers[module_id] = model.model.layers[module_id].to(device1)
        # module_copy = copy.deepcopy(model.model.layers[module_id])
        # module_copy = module_copy.to(device1)
        LlamaModel.forward = create_VPA_offload_forward(module_id,device0,device1)
        part1 = time.time()
        deploy = part1 - start
        ...
        history_dict = {}
        chat(model=model,tokenizer=tokenizer,prompts=prompts,batch_size = 10,max_new_tokens=64,history_dict=history_dict,device=device0)
...
with open('HPA.txt', 'w') as file:
    file.write(f"==============llama2-13b HPA===================\n")
    file.write(f"xlabel = {list(range(10, 121, 10))}\n")
