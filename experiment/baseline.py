...
# device_map = {"": "cuda:0"}
for j in list(range(100, 151, 10)):
    all_time_1 = []
    deploy_time_1 = []
    inference_time_1 = []
    for i in range(5):
        ...
        history_dict = {}
        chat(model=model,tokenizer=tokenizer,prompts=prompts,batch_size = 5,max_new_tokens=64,history_dict=history_dict,device=device0)
...
with open('baseline.txt', 'w') as file:
    file.write(f"==============llama2-13b===================\n")
    file.write(f"xlabel = {list(range(5, 451, 5))}\n")
